...
| Stolen admin session token | Session is server-side revocable (manually, via logout); token itself is hashed at rest; `Secure`/`HttpOnly`/`SameSite` cookie flags limit exfiltration paths. Changing password/TOTP does not yet auto-revoke *other* sessions for the same admin — tracked as issue #24. |
